...
class BlocketSpiderMiddleware:
    ...
    def process_spider_input(self, response, spider):
        ...
        self._mark_url_in_progress(response)  # Отметка о начале обработки
        return None
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if parent_url:
            yield self.lock.run(self._decrement_count, parent_url)

    def _initialize_or_increment(self, parent_url, result):
        """Инициализация или увеличение счетчика дочерних запросов для parent_url."""
        if parent_url not in self.children_request_counts:
            # Инициализация счетчика на основе количества запросов в результате
            self.children_request_counts[parent_url] = sum(1 for item in result if isinstance(item, scrapy.Request))
            print(f"Инициализирован счетчик для {parent_url}: {self.children_request_counts[parent_url]}")
        else:
            self.children_request_counts[parent_url] += sum(1 for item in result if isinstance(item, scrapy.Request))

    def _decrement_count(self, parent_url):
        """Уменьшает счетчик дочерних запросов для указанного parent_url."""
        if parent_url in self.children_request_counts:
            self.children_request_counts[parent_url] -= 1
            if self.children_request_counts[parent_url] == 0:
                # Помечаем URL как обработанный и удаляем из словаря
                self._mark_url_processed(parent_url)
                del self.children_request_counts[parent_url]

    def _mark_url_in_progress(self, response: Response):
        """Записывает URL как "в процессе" в базе данных."""
        url = response.url
        ...
        try:
            ...
            VALUES (?, ?, "in_progress") 
            ON CONFLICT(url) DO UPDATE SET status="in_progress"
            ''',
                           (fp, url,))
    ...
    def _mark_url_processed(self, url):
        """Отмечает URL как обработанный в базе данных."""
        ...
        try:
            ...
            WHERE fingerprint = ?
            ''',
            (last_processed_date, fingerprint,))
            self.connection.commit()
        except sqlite3.Error as e:
